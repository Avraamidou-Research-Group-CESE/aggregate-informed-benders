...
"""
Function to run the model as a 3-state problem (on/off/sb) for RTM
"""
function run_3st_opt_rtm(θ::Electrolyzer, spp_array; gap=0.01, z_fixed=nothing)
    ...
    T_star = [t for (i, t) in enumerate(T_span) if i % (8760*n_per_hour) == 0] # 720 --> for once a month in DAM, 2880 --> once a month in 15-RTM
    ...
    year_indices = [((y-1)*8760*n_per_hour + 1):(y*8760*n_per_hour) for y in years]
    year_indices[end] = ((n_years-1)*8760*n_per_hour+1):(length(spp_avg_arr)) # in case spp_arr is not exactly 22*8760
    ...
    n_hours_per_day = 24
    total_days = Int.(floor(length(T_span)/(n_hours_per_day*n_per_hour)))
    D_H = 750 #sigma
    daily_indices = [
        T_span[(d-1)*n_hours_per_day*n_per_hour + 1 : d*n_hours_per_day*n_per_hour]
    ...
    @constraint(model, z_start[1] >= z_on[1])
    ...
    if z_fixed !== nothing
        for y in years
            fix(model[:z_replace][y], z_fixed[y]; force=true)
        end
    end
...
"""
function run_3st_opt(θ::Electrolyzer, spp_array; gap=0.01, z_fixed=nothing)
    model = Model(Gurobi.Optimizer)
    ϕ = θ.ϕ # nominal capacity
    α_max = θ.α_max
    α_init = α_max
    ...
    # first hours of years 2..n_years: replacement resets efficiency at start of each year
    T_star = [8760*(y-1)+1 for y in 2:n_years]
    ...
    @constraint(model, A[1] == α_init - δ_on * z_on[1] * ΔT - δ_start * z_start[1]) # IC
    @constraint(model, [t in T_subset], A[t] == A[t-1] - δ_on * z_on[t] * ΔT - δ_start * z_start[t])
    @constraint(model, [(y,t) in zip(2:n_years, T_star)], A[t] == A[t-1] - v[y] - δ_on * z_on[t] * ΔT - δ_start * z_start[t] + α_max * z_replace[y]) # replacements


    # Bilinear term linearization (v_t = A_{t-1} * z_replace_{t} ) # if replace = 1 --> A[t-1] cancels out, otherwise =0
    @constraint(model, z_replace[1] == 0)
    @constraint(model, [y in years], v[y] <= α_max * z_replace[y])
    @constraint(model, [y in years], v[y] >= α_min * z_replace[y])
    @constraint(model, [(y,t) in zip(2:n_years, T_star)], v[y] <= A[t-1] - α_min * (1 - z_replace[y]))
    @constraint(model, [(y,t) in zip(2:n_years, T_star)], v[y] >= A[t-1] - α_max * (1 - z_replace[y]))
